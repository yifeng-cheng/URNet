...
class MLLARefinement(nn.Module):
    r""" MLLA Refinement
        A PyTorch implementation for refinement task instead of classification.
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3,
        ...
        self.refinement_head = nn.Sequential(
            nn.Conv2d(self.num_features, embed_dim, kernel_size=3, padding=1),
            nn.Conv2d(embed_dim, in_chans, kernel_size=3, padding=1)  # 输出维度与输入一致
        )
    ...
    def forward_features(self, x):
        ...
        x = self.norm(x)  # B L C
        x = x.transpose(1, 2).reshape(x.size(0), self.num_features, self.patches_resolution[0], self.patches_resolution[1])
        return x
    
    
    def forward(self, x):
        x = self.forward_features(x)
        x = self.refinement_head(x)
