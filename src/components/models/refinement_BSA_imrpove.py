...
class RefineAttention(nn.Module):
    ...
    def forward(self, x, mask_d, mask_e):
        ...
        # 调整 mask_d 和 mask_e 的尺寸与 x 一致
        mask_d = F.interpolate(mask_d, size=(h, w), mode='bilinear', align_corners=False)
        mask_e = F.interpolate(mask_e, size=(h, w), mode='bilinear', align_corners=False)
