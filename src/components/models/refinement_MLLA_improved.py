...
class RefUnet(nn.Module):
    ...
    def __init__(self, in_ch, inc_ch):
        ...
        self.mlla_dim = 64  # 与特征通道数一致

        self.mlla_heads = 4  # 设置注意力头数
    ...
    def forward(self, x):
        ...
        hx5 = self.relu5(self.bn5(self.conv5(hx)))  # [B, 64, H, W]
        ...
        x_mlla = self.proj_in(hx5)  # 如果需要调整通道数

        x_mlla = x_mlla.permute(0, 2, 3, 1).reshape(B, H * W, self.mlla_dim)  # [B, L, C]

 

        x_mlla = self.mlla_block(x_mlla, H, W)  # [B, L, C]

 

        x_mlla = x_mlla.reshape(B, H, W, self.mlla_dim).permute(0, 3, 1, 2)  # [B, C, H, W]

        x_mlla = self.proj_out(x_mlla)  # 如果需要恢复通道数
