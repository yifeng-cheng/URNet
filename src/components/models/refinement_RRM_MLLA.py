...
from .mlla import MLLARefinement  # 假设 MLLA 定义在 mlla 模块中
...
class StereoDRNetRefinement(nn.Module):
    def __init__(self, img_channels=3):
        ...
        # 初始化 MLLA 模块
        self.mlla = MLLARefinement(img_size=(216, 288),  # 输入图像的尺寸，可以根据输入的实际尺寸进行调整
                                   patch_size=4,  # 由于 disp 可能是单通道图像，这里使用较小的 patch size
                                   in_chans=1,    # disp 是单通道图像
                                   num_classes=1,  # 这里只需要生成一个通道的输出
                                   embed_dim=32,   # 可以根据需要调整
                                   depths=[2],    # 使用一个浅层的 MLLA 模块
                                   num_heads=[4],  # 头数设置为 4
                                   mlp_ratio=4.0,  # 默认 MLP 比例
                                   qkv_bias=True, 
                                   drop_rate=0., 
                                   drop_path_rate=0.1)
    ...
    def forward(self, low_disp, left_img, right_img):
        ...
        disp_resized = F.interpolate(disp, size=(216, 288), mode='bilinear', align_corners=False)

        disp = self.mlla(disp_resized)
    
        disp = F.interpolate(disp, size=residual_disp.shape[-2:], mode='bilinear', align_corners=False)
        
        # RefUnet 处理
        refined_disp = self.refunet(disp)

        # Combine the refined disp with residual
        disp = F.relu(refined_disp + residual_disp, inplace=True)  # [B, 1, H, W]
