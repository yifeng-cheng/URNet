...
def disp_warp(img, disp, padding_mode='border'):
    ...
    Returns:
        ...
        valid_mask: [B, 3, H, W]
    """
    # Check if disp contains any negative values
    if disp.min() < 0:
        # If there are negative values, skip this operation
        print("Warning: disp contains negative values. Skipping this warp operation.")
        return img, torch.zeros_like(img)  # Return original image and a zero mask
